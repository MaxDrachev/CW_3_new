...
from src.utils import open_json, filter_operation, sort_operations, hide_number_from, hide_number_to, \
    new_date

PATH_TO_TEST_FILE = os.path.join(ROOT_DIR, 'tests', 'test_file.json')


def test_open_json():
    assert type(open_json(PATH_TO_TEST_FILE)) == list
    assert open_json(PATH_TO_TEST_FILE) == [
        {
            "1": "3"
        },
        {
...
def test_sort_operations():
    ...
    assert sort_operations(test_list)[0] == {
            "id": 172864002,
            "state": "EXECUTED",
            "date": "2018-12-28T23:10:35.459698",
            "operationAmount": {
